...
if(isset($_POST['submit']))
{
    ...
    $ret=mysqli_query($con->bd,"SELECT * FROM doctors WHERE docEmail='".$_POST['username']."' and password='".($_POST['password'])."'");
...
$status=1;
$log=mysqli_query($con->bd,"insert into doctorslog(uid,username,userip,status) values('".$_SESSION['id']."','".$_SESSION['dlogin']."','$uip','$status')");
...
$uip=$_SERVER['REMOTE_ADDR'];
$status=0;
mysqli_query($con->bd,"insert into doctorslog(username,userip,status) values('".$_SESSION['dlogin']."','$uip','$status')");
$_SESSION['errmsg']="Nome ou senha invalidos!";
$extra="index.php";
$uri  = rtrim(dirname($_SERVER['PHP_SELF']),'/\\');
header("location:http://$host$uri/$extra");
exit();
